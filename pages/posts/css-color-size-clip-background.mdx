export const meta = {
    ...
    slug: '/posts/css-color-size-clip-background'
}
...
CSS 中的颜色值有多种表示方式，常见方式是：关键字（pink）、十六进制（#D4838F）、RGB（rgba(234, 79, 108, 1)）和 HSL（hsla(337, 50%, 42%, 1)）。在这里重点要介绍的是 HSL 的表示方式，这里的 HSL 分别是指色相（Hue, H）、饱和度（Saturation, S）和明度（Lightness, L）。为什么要强调这种方式呢？因为它可以更好的记忆：

![/assets/color-wheel-hsl-fixed-lightness-three.png](/assets/color-wheel-hsl-fixed-lightness-three.png)
